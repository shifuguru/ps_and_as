...
export default function GameScreen({ initialPlayers }: { initialPlayers?: string[] } = {}) {
    ...
    <View style={local.container}>
      ...
      <View style={local.pileArea}>
        ...
        <View style={local.tableBorder}>
          <View style={local.pileCards}>
            {state.pile.length === 0 ? (
              <Text style={{ color: "#ccc" }}>No cards on the table</Text>
            ) : (
              state.pile.map((c, i) => (
                <View key={`${c.suit}-${c.value}-${i}`} style={local.pileCardWrapper}>
      ...
      <View style={[local.bottomBar, { minHeight: bottomBarMinHeight }] }>
                ...
                <Card
                  ...
                  onPress={() => {
                    ...
                    if (currentSelected.length === 0) {
                      const same = hand.map((h, i) => (h.value === tappedValue ? i : -1)).filter((x) => x !== -1);
                      setSelected(same as number[]);
                    } else {
        ...
        <TouchableOpacity
          ...
          style={[local.actionButton, { marginLeft: 12 }]}
        >
