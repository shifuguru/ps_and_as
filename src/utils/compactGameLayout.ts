...
const BASE_FAN_HEADROOM = 50; // SELECT_LIFT + MAX_CENTER_LIFT + 24
...
export function resolveHandMetrics(shellHeight: number): HandLayoutMetrics {
  ...
  const fanHeight =
    cardHeight + Math.round(BASE_FAN_HEADROOM * fanHeadroomScale(tier));
...
export function resolveBottomChromeMetrics(
  ...
  const reservedHeight =
    8 + handSection + actionBarHeight + actionBarPadding + 4 + bottomOuterPad;
