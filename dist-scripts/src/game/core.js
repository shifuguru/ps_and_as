...
exports.RANK_ORDER = exports.SINGLE_RANK_PER_TURN = void 0;
...
exports.runFromCurrentTrickInfo = runFromCurrentTrickInfo;
exports.runFromCurrentTrick = runFromCurrentTrick;
exports.isValidPlay = isValidPlay;
exports.isRunContextSequence = isRunContextSequence;
...
function effectivePile(pile, pileHistory) {
    try {
        ...
        if (collected.length >= 3 && isRunContextSequence(collected)) {
            try {
                console.log(`[core] effectivePile detected run (context) from pileHistory: ${collected.map(c => c.value).join(',')}`);
            }
            catch (e) { }
            return collected;
        }
...
function runFromCurrentTrickInfo(currentTrick, players, finishedOrder = []) {
...
function isValidPlay(cards, pile, tenRule, pileHistory, trickHistory, fourOfAKindChallenge, currentTrick, players, finishedOrder) {
    ...
    if (pileCount === 0 &&
        ...
        if (!hasThreeClubs)
            return false;
        if (!allSameValue(cards) || cards[0].value !== 3)
            return false;
    ...
    if (isSingleJoker(cards) && pileCount > 0) {
        if (isPileRun)
            return false; // Jokers not allowed in runs
        if (pile.some(c => isJoker(c)))
            return false; // Joker can't beat Joker
        if (tenRule?.active && tenRule.direction === 'lower')
            return false; // Joker can't beat 10-lower
        return true;
    }
...
function isRunContextSequence(cards) {
    ...
    for (let i = 0; i < cards.length; i++) {
        const c = cards[i];
        if (isJoker(c))
            return false; // Jokers are not allowed in runs
    }
...
function findCPUPlay(hand, pile, tenRule, pileHistory, fourOfAKindChallenge, currentTrick, players, finishedOrder) {
            ...
            for (const v of values) {
                ...
                if (cards.length >= runMultiplicity) {
                    ...
                    if (Math.abs(r - lastRank) === 1)
                        return cards.slice(0, runMultiplicity);
                }
            }
            return null;
        }
    ...
    if (fourOfAKindChallenge?.active) {
        ...
        if (jokerCard && !pile.some(c => isJoker(c)))
            return [jokerCard];
        return null;
    }
